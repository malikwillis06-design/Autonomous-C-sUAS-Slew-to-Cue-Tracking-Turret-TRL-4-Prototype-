...
HORIZONTAL_FOV_DEG = 67.0
VERTICAL_FOV_DEG = 53.7
...
# Step limits scale with how far off-target we are: small steps near
# lock for precision, larger steps for big corrections so the turret
# doesn't lag behind fast motion. Lowered from the first pass since
# 18 PWM/frame was too aggressive at this loop's effective frame rate.
MAX_STEP_SMALL = 2.0
MAX_STEP_LARGE = 8.0
LARGE_ERROR_THRESHOLD_PWM = 25.0
...
def calculate_servo_step(
    ...
    step_limit = (
        MAX_STEP_LARGE
        if abs(error_pwm) > LARGE_ERROR_THRESHOLD_PWM
        else MAX_STEP_SMALL
    )
